...
class NexusPipeline:
    ...
    async def run(self, task_id: str, query: str, user_id: str,
        ...
        try:
            ...
            if cls.query_type == QueryType.ACTION:
                ...
                result = await self._run_knowledge(task_id, effective_query, user_id, cls, emit, t0)
    ...
    async def _run_live(self, task_id, query, user_id, cls, emit, t0):
            ...
            if not result_data.flights:
                ...
                r = await llm_client.chat(
                    ...
                    messages=[{"role":"user","content":
                        ...
                        f"Give a clear recommendation: cheapest option with details, and 2-3 alternatives. "
                        f"Format for user. Include price, airline, duration if available."}],
        ...
        # ── Commodity (gold, silver, oil) ────────────────────────────────────
            ...
            from src.tools.live_engine import get_commodity_live
            commodity = entities.get("commodity","gold")
            loc_str = f"{city} {country}" if city else country
            emit("scrape", f"Fetching live {commodity} price for {loc_str} — 3-tier: API → Playwright → Groq")
            price_data = await get_commodity_live(commodity=commodity, city=city, country_code=country)
            emit("scrape", f"Verified {len(price_data.sources)} sources · spread {price_data.spread_pct:.2f}%")
            ...
            if price_data.error:
                ...
                r = await llm_client.chat(
                    ...
                    messages=[{"role":"user","content":
                        ...
                        f"Give a clear, direct answer with the price, location, time, and any relevant context "
                        f"(trend, comparison to yesterday if available). Be concise."}],
        ...
        # ── Crypto ────────────────────────────────────────────────────────────
            ...
            from src.tools.live_engine import get_crypto_live
            symbol = entities.get("crypto_symbol","BTC")
            emit("scrape", f"Fetching live {symbol} price from CoinGecko (free, no key)")
            data_obj = await get_crypto_live(symbol)
            data = {"symbol":data_obj.query,"price_usd":data_obj.numeric,
                    "price_inr":None,"change_24h":0,"source":"coingecko.com"} if data_obj.numeric else None
            if not data:
        ...
        # ── Stock index ───────────────────────────────────────────────────────
            ...
            from src.tools.live_engine import get_stock_live
            emit("scrape", "Fetching live stock index from NSE India official API")
            idx_query = re.search(r"\b(nifty|sensex|nasdaq|dow|s&p|ftse|nikkei)\b", query, re.I)
            index_name = idx_query.group(1).upper() if idx_query else "NIFTY"
            index_name = "NIFTY 50" if "NIFTY" in index_name else index_name
            stock_obj = await get_stock_live(index_name)
            if stock_obj.error or not stock_obj.numeric:
                verdict = f"Could not fetch {index_name} right now. {stock_obj.error or ''} Check nseindia.com"
                conf = 0
            else:
                verdict = (f"**{stock_obj.query}**: {stock_obj.display}\n"
                           f"Source: {stock_obj.sources[0].source if stock_obj.sources else 'NSE'}"
                           f" · {stock_obj.as_of}")
                conf = 0.99
            return PipelineResult(
                task_id=task_id, query=query, subtype="stock",
                verdict=verdict, confidence=conf,
                sources=[stock_obj.sources[0].source if stock_obj.sources else "nseindia.com"],
        ...
        # ── Weather ───────────────────────────────────────────────────────────
            ...
            from src.tools.live_engine import get_weather_live
            loc = city or "Bengaluru"
            emit("scrape", f"Fetching weather for {loc} from open-meteo.com (free, real-time)")
            w_obj = await get_weather_live(loc, country)
            w_legacy = type("W",(),{"city":w_obj.location,"temp_c":w_obj.numeric or 0,
                                     "feels_like_c":w_obj.numeric or 0,"humidity":0,
                                     "wind_kmh":0,"description":w_obj.display,
                                     "source":"open-meteo.com"})()
            w = w_legacy if w_obj.numeric else None
            if not w:
    ...
    # ── Knowledge pipeline ──────────────────────────────────────────────────────
    async def _run_knowledge(self, task_id, query, user_id, cls, emit, t0):
        emit("research", "Searching knowledge base + web")

        # Web search for current info
        from src.tools.task_executor import task_executor
        web_results = None
        if any(w in query.lower() for w in ["latest","recent","news","2024","2025","who is","what happened"]):
            sr = await task_executor.execute("web_search", {"query": query, "num": 5})
            if sr.success: web_results = sr.data

        # Memory search
        try:
            from src.memory.vector_store import vector_memory
            mem_results = await vector_memory.search(query, top_k=4)
        except: mem_results = []
        ...
        r = await llm_client.chat(
            ...
            messages=[{"role":"user","content":
                f"Question: {query}\n\n{context_str}\n\n"
                f"Give a comprehensive, accurate answer. Cite sources where available."}],
